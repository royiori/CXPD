...
void MyEventClass::AnalysisHist()
{
    ...
    //2.5 cut the fatty events
    {
        ...
        {
			if ((maxx - minx)/(maxy - miny) < nEllipticity && (maxx - minx)/(maxy - miny) > 1/nEllipticity){ //判断fat事例的圆形程度
			dataQFlag = QF_FAT;
...
//______________________________________________________________________________
void MyEventClass::Draw2DResult(const char *opt)
{
	if (f2D != NULL)
	    f2D->Draw(opt);
	if (mBcenter != NULL)
